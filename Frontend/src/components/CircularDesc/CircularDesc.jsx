        ...
        <div className="wrapper-desc">
            <div className="circ-cont">
                {descData.map((item,index) => {
                    return(<div className='bloods' key={item.label} style={{ "--angle":index*45+"deg",
                    background:`${item.label==desc.label?'black':''}`,
                    border:`${item.label==desc.label?'1px solid red':''}`,
                    color:`${item.label==desc.label?'#ccc':''}`
